...
function _fetchEachEvent(categoryIdToFind, publishId) {
  ...
  const event = fetchEvent.find(
    (s) =>
      s.page_category_id === categoryIdToFind && s.publish_id === publishId,
  );
          ...
          if (success === true) {
            if (!fetch) {
              ...
              for (let i = 0; i < fetch.length; i++) {
                ...
                const class_gallery_sub_title =
                  fetch[i].class_gallery_sub_title;
                const updated_time = fetch[i].updated_time;
                const status_name = fetch[i].status_name;
                const reg_pix = fetch[i].reg_pix;
                const documentStoragePath = fetch[i].documentStoragePath;
                const formattedDate = formatDate(updated_time);
                const page_view = fetch[i].page_view;
                const displayTitle = gallery_sub_title
                  ? gallery_sub_title
                  : class_gallery_sub_title;
...
function _addGallery(page_category_id) {
  try {
    ...
    $(
      "#reg_title, #gallery_sub_title, #class_gallery_sub_title, #reg_thumbnail, #status_id",
    ).removeClass("issue");
