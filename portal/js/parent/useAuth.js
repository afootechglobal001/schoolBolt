...
function _proceedViewStudentCaResult(formData) {
  ///// get btn text/////
  ...
  _callRawEndPoints({
    ...
    .catch((error) => {
      console.error("Error:", error);
      _callAjaxError(() => _proceedViewStudentCaResult(formData)); // retry if needed
      _btnDisable("proceedResult", btnText, false);
    });
}

/// Proceed To View Student Terminal Result ///
function _proceedViewStudentTerminalResult(formData) {
  ///// get btn text/////
  const btnText = $("#proceedResult").html();
  _btnDisable("proceedResult", btnText, true);
