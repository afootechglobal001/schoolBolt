...
function _loadStudentFund(){
	try {
		...
		let issueCount = 0;
		const amountReceivedFromParent = $('#amountReceivedFromParent').val().trim();
		const description = $('#description').val().trim();
    	
		///// empty field validation//////////
		issueCount += _validateEmptyValue("amountReceivedFromParent", "AMOUNT");
		issueCount += _validateNumber("amountReceivedFromParent", amountReceivedFromParent);
		...
		.then((response) => {
			...
			if (response.success) {
				_showCustomConfirm({
					callback: () => {
						_alertClose(3);
						_fetchAccountStudentsByClass(departmentId, classId, armId, branchId, session, termId)				
					},
					title: 'Success!',
