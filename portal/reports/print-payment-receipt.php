...
<head>
    ...
    <script>
        const fileStudentName = printGeneralPaymentRecieptBreakdownSession?.studentData?.surName + '-' +
        printGeneralPaymentRecieptBreakdownSession?.studentData?.firstName + '-' +
        printGeneralPaymentRecieptBreakdownSession?.studentData?.otherNames;

        const fileTermName= printGeneralPaymentRecieptBreakdownSession?.termData?.termName;

        function _downloadLetter(){
            let element = document.querySelector(".recieptContent");
            let options = {
                margin: [0.0, -0.9000, 0.0, 0.0],
                filename: `${capitalizeFirstLetterOfEachWord(fileStudentName)}-${capitalizeFirstLetterOfEachWord(fileTermName)}-Payment-Reciept.pdf`,
                image: {
                    type: 'jpeg',
                    quality: 1
                },
