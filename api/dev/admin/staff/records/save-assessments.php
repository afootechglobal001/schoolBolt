...
if(!$checkSession){
    ...
    $studentScoreQuery = mysqli_query($conn, "SELECT DISTINCT(studentId) AS studentId, SUM(markObtained) AS totalMark, COUNT(studentId) AS numberOfSittings FROM BRANCH_ASSESSMENT_RECORD_DETAILS_TAB WHERE recordId IN ($recordIds) GROUP BY studentId") or die(mysqli_error($conn));
    ...
    while ($studentScoreFetch = mysqli_fetch_assoc($studentScoreQuery)) {
        ...
        $numberOfSittings = $studentScoreFetch['numberOfSittings'];
        ...
        (`clientId`, `branchId`, `session`, `termId`, `departmentId`, `classId`, `armId`, `subjectId`, `studentId`, `numberOfSittings`, `allAssessmentTotalMark`, `grade`, `remark`) VALUES
        ('$clientId', '$branchId', '$session', '$termId', '$departmentId', '$classId', '$armId', '$subjectId', '$studentId', '$numberOfSittings', '$totalMark', '$grade', '$remark')")or die (mysqli_error($conn));
    }


    /// update students position in arm per subject in the term
    $markChecker=0;
    $count=0;
    ...
    ///// update BRANCH_ASSESSMENT_NEW_UPDATE_ALERT_TAB for this class terminal broad sheet
    $getAlertQuery = mysqli_query($conn, "SELECT * FROM BRANCH_ASSESSMENT_NEW_UPDATE_ALERT_TAB WHERE $clientIds AND branchId = '$branchId' AND session = '$session' AND termId = '$termId' AND departmentId = '$departmentId' AND classId = '$classId' AND armId = '$armId' LIMIT 1") or die(mysqli_error($conn));
    if (mysqli_num_rows($getAlertQuery) > 0) {
        mysqli_query($conn, "UPDATE BRANCH_ASSESSMENT_NEW_UPDATE_ALERT_TAB 
        SET updatedBy = '$loginStaffId', createdTime = NOW() 
        WHERE $clientIds AND branchId = '$branchId' AND session = '$session' AND termId = '$termId' AND departmentId = '$departmentId' AND classId = '$classId' AND armId = '$armId'") or die(mysqli_error($conn));
    } else {
        mysqli_query($conn, "INSERT INTO BRANCH_ASSESSMENT_NEW_UPDATE_ALERT_TAB 
        (clientId, branchId, session, termId, departmentId, classId, armId, updatedBy, createdTime) VALUES 
        ('$clientId', '$branchId', '$session', '$termId', '$departmentId', '$classId', '$armId', '$loginStaffId', NOW())") or die(mysqli_error($conn));
    }
