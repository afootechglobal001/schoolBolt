...
if(!$checkSession){
    ...
    if(empty($statusId)){
        ...
        if(!filter_var($emailAddress, FILTER_VALIDATE_EMAIL)){
			...
			$query=mysqli_query($conn,"SELECT * FROM STAFF_TAB WHERE $clientIds AND emailAddress='$emailAddress' AND staffId!='$staffId'") or die (mysqli_error($conn));
            ...
            `dateOfBirth`='$dateOfBirth', `stateId`='$stateId', `lgaId`='$lgaId', `address`='$address', `updatedBy`='$loginStaffId', `updatedTime`=NOW() WHERE $clientIds AND staffId='$staffId'")or die (mysqli_error($conn));
