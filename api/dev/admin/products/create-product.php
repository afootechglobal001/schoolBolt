...
if(!$checkSession){
    ...
    $productPixCount = count($_FILES["productPix"]["name"]);
    ...
    if(empty($statusId)){
        ...
        goto end;
			...
			$query=mysqli_query($conn,"SELECT * FROM PRODUCTS_TAB WHERE productLevel=2 AND  productName='$productName'") or die (mysqli_error($conn));
            ...
            (`parentId`, `productLevel`, `productId`, `productName`, `productTags`, `statusId`, `createdBy`, `createdTime`) VALUES
            ('$parentId', 2,'$productId','$productName', '$productTags',  '$statusId', '$loginStaffId', NOW())")or die (mysqli_error($conn));
