...
if(!$checkSession){
    ...
    if (empty($statusId)){/// start if 2
        ...
        goto end;
			...
			$query=mysqli_query($conn,"SELECT * FROM ARMS_TAB WHERE $clientIds AND armName='$armName' AND armId!='$armId'") or die (mysqli_error($conn));
            ...
            WHERE $clientIds AND armId = '$armId'")or die (mysqli_error($conn));
