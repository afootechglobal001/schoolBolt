...
if(!$checkSession){
	...
	if (empty($armId)){/// start if 2
        $response = [
            'response'=> 100,
            'success'=> false,
            'message'=> "ARM ID REQUIRED! Check the fields and try again",
        ]; 
        goto end;
	}
    ...
    if (empty($statusId)){/// start if 2
        ...
        goto end;
			...
			$query=mysqli_query($conn,"SELECT * FROM ARMS_TAB WHERE armName='$armName' AND armId!='$armId'") or die (mysqli_error($conn));
            ...
            mysqli_query($conn,"UPDATE `ARMS_TAB` SET 
            armName='$armName', statusId='$statusId', updatedBy='$loginStaffId'
            WHERE armId = '$armId'")or die (mysqli_error($conn));

            $response['response']=200; 
            $response['success']=true;
            $response['message']="ARM UPDATED SUCCESFFULY!"; 
            $response['data'] = array(); // Initialize the data array

            $select="SELECT * FROM ARMS_TAB WHERE armId = '$armId'";
