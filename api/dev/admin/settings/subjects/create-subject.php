...
if(!$checkSession){
    ...
    if (empty($statusId)){/// start if 2
        ...
        goto end;
			...
			$query=mysqli_query($conn,"SELECT * FROM SUBJECTS_TAB WHERE $clientIds AND subjectName='$subjectName'") or die (mysqli_error($conn));
            ...
            if ($count>0){ /// start if 4
                $response = [
                    'response'=> 103,
                    'success'=> false,
                    'message'=> "SUBJECT EXIST! Subject already exist by name. Check and try again.",
                ];
                goto end;
            }
            ...
            (`subjectId`, `subjectName`,  `createdBy`, `statusId`, `createdTime`) VALUES 
            ('$subjectId','$subjectName', '$loginStaffId', '$statusId', NOW() )")or die (mysqli_error($conn));

            $response['response']=200; 
            $response['success']=true;
            $response['message']="SUBJECT CREATED SUCCESFFULY!"; 
            $response['data'] = array(); // Initialize the data array

            $select="SELECT * FROM SUBJECTS_TAB WHERE $clientIds AND subjectId = '$subjectId'";
