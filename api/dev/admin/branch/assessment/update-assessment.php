...
if(!$checkSession){
    ...
    if (empty($assessmentId)){/// start if 2
        $response = [
            'response'=> 100,
            'success'=> false,
            'message'=> "ASSESSMENT ID REQUIRED! Check the fields and try again",
        ]; 
        goto end;
	}
    ...
    if($assessmentTotalScore<0){
        ...
        goto end;
			...
			$query=mysqli_query($conn,"SELECT * FROM BRANCH_ASSESSMENT_SETUP_TAB WHERE $clientIds AND branchId='$branchId' AND assessmentName='$assessmentName' AND assessmentId!='$assessmentId'") or die (mysqli_error($conn));
            ...
            $sumQuery = mysqli_query($conn, "SELECT SUM(assessmentTotalScore) AS totalScore FROM BRANCH_ASSESSMENT_SETUP_TAB WHERE $clientIds AND branchId='$branchId'AND (parentId IS NULL OR parentId = '') AND assessmentId!='$assessmentId'") or die (mysqli_error($conn));
            ...
            mysqli_query($conn,"UPDATE `BRANCH_ASSESSMENT_SETUP_TAB` SET
             `assessmentName`='$assessmentName', `assessmentTotalScore`='$assessmentTotalScore', `updatedBy`='$loginStaffId'
             WHERE $clientIds AND branchId='$branchId' AND assessmentId='$assessmentId'")or die (mysqli_error($conn));
           ...
           $select="SELECT * FROM BRANCH_ASSESSMENT_SETUP_TAB WHERE $clientIds AND branchId = '$branchId' AND (parentId IS NULL OR parentId = '') AND assessmentId='$assessmentId'";
            ...
            $response['message']="ASSESSMENT UPDATED SUCCESFFULY!";
