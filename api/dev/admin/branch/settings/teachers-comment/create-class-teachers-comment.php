...
if(!$checkSession){
    ...
    validateEmptyField($statusId, 'STATUS');
			...
			$query=mysqli_query($conn,"SELECT * FROM BRANCH_TEACHERS_COMMENT_CONFIG_TAB WHERE $clientIds AND branchId='$branchId' AND genderId='$genderId' AND comment='$comment'") or die (mysqli_error($conn));
            ...
            $no= $array[0]['no'];
            $commentId=$countId.$no.date("Ymdhis");
            mysqli_query($conn,"INSERT INTO `BRANCH_TEACHERS_COMMENT_CONFIG_TAB`
            (`clientId`, `branchId`, `commentId`, `genderId`, `comment`, `statusId`, `createdBy`, `createdTime`) VALUES
            ('$clientId','$branchId','$commentId', '$genderId', '$comment', '$statusId', '$loginStaffId', NOW())")or die (mysqli_error($conn));
