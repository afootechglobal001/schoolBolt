...
if(!$checkSession){
    ...
    $select="SELECT a.*, b.* FROM STUDENTS_CLASS_TAB a, STUDENT_VIEW b WHERE a.clientId=b.clientId AND a.branchId=b.branchId AND a.studentId=b.studentId  AND  a.clientId='$clientId' AND a.branchId = '$branchId' 
    AND (b.studentId LIKE '%$q%' OR b.surName LIKE '%$q%' OR b.firstName LIKE '%$q%' OR b.otherNames LIKE '%$q%' OR b.address LIKE '%$q%' OR b.email LIKE '%$q%' OR b.mobileNumber LIKE '%$q%' OR b.sessionRegistered LIKE '%$q%' OR b.genderName LIKE '%$q%' OR b.countryName LIKE '%$q%'  OR b.stateName LIKE '%$q%' OR b.lgaName LIKE '%$q%' OR b.maritalStatusName LIKE '%$q%') $studentIds  $statusIds ORDER BY b.surName ASC";
