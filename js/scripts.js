...
function _getActiveNextModulePage(text) {
  $(
    "#next-admin, #next-staff, #next-student, #next-bursary, #next-parents"
  ).removeClass("active");
  $("#next-" + text).addClass("active");
}
...
function _nextModulePage(next_id, text) {
  ...
  $(
    "#admin-hide-div, #staff-hide-div, #student-hide-div, #bursary-hide-div, #parents-hide-div"
  ).hide();
  $("#" + next_id).fadeIn(1000);
}
...
function _setupShowMore(containerSelector, buttonId, maxItems) {
  ...
  $button.on("click", function () {
    ...
    $items.each(function (index) {
      $(this).css(
        "display",
        isShowingMore && index >= maxItems ? "" : index < maxItems ? "" : "none"
      );
    });
  });
}
